...
CREATE TABLE Tripulantes (
    id_tripulante INT PRIMARY KEY IDENTITY(1,1),
...
-- Tabela EquipeMissoes (Relacionamento Missão ↔ Tripulantes)
CREATE TABLE EquipeMissoes (
    id_missao INT,
    id_tripulante INT,
    funcao_na_missao VARCHAR(100),
    PRIMARY KEY (id_missao, id_tripulante),
    FOREIGN KEY (id_missao) REFERENCES Missoes(id_missao) ON DELETE NO ACTION ON UPDATE NO ACTION,
    FOREIGN KEY (id_tripulante) REFERENCES Tripulantes(id_tripulante) ON DELETE NO ACTION ON UPDATE NO ACTION
);
...
INSERT INTO EquipeMissoes (id_missao, id_tripulante, funcao_na_missao)
SELECT 
    m.id_missao,
    t.id_tripulante,
    t.competencia
